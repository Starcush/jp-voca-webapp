...
import { AppFrameContent } from "@/components/AppFrameContent";
import { AppFrameLanguageMenu } from "@/components/AppFrameLanguageMenu";
import {
  AppNavigation,
  AppNavigationFallback,
} from "@/components/AppNavigation";
import { AppRouteTransitionProvider } from "@/components/AppRouteTransition";
    ...
    <main className="min-h-dvh bg-brand-background px-4 pb-16 pt-0 text-brand-text md:bg-white md:p-0">
      <div className="grid min-h-dvh w-full md:grid-cols-[216px_minmax(0,1fr)]">
        <AppRouteTransitionProvider>
          ...
          <AppFrameContent header={header}>{children}</AppFrameContent>
        </AppRouteTransitionProvider>
      </div>
    </main>
  );
