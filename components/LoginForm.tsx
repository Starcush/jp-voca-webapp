...
export function LoginForm() {
  ...
  const isLoginTransitioning =
    ...
    submittingMode === "sign-up";

  const navigateAfterLogin = useCallback((nextSession: {
    ...
    return onAuthStateChanged(getFirebaseAuth(), (user) => {
      ...
      void createSessionFromFirebaseUser(user, true)
        .then((restoredSession) => {
          ...
          navigateAfterLogin(restoredSession);
        })
        .catch((error) => {
  ...
  return (
    <>
      <LoadingOverlay
        message={isSignUp ? "계정을 준비하는 중" : "단어장을 여는 중"}
        show={isLoginTransitioning}
      />
      <form
