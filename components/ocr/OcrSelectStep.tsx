  ...
  return (
    <section className="grid gap-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <button
          className="min-h-10 rounded-lg border border-brand-border bg-white px-3 text-sm font-bold text-brand-muted"
          onClick={onBack}
          type="button"
        >
          사진/텍스트 수정
        ...
        <button
          className="min-h-10 rounded-lg bg-primary px-3 text-sm font-black text-white disabled:cursor-not-allowed disabled:bg-brand-muted-soft"
          disabled={!canConfirmExpressions}
          onClick={onConfirm}
          type="button"
        >
          저장 전 확인 {stagedExpressionCount > 0 ? stagedExpressionCount : ""}
        </button>
      </div>
      <SentenceSelector
